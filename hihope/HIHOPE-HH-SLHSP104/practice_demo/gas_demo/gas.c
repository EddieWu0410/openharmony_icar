...
void gas_init(void)
{
    ...
    // 配置GPIOA4为模拟输入模式（可燃气体传感器引脚）
    gpio_default_para_init(&gpio_init_struct);
    gpio_init_struct.gpio_mode = GPIO_MODE_ANALOG;
    gpio_init_struct.gpio_pins = GPIO_PINS_4;  // 注意：气体传感器使用PA4（光照传感器用PA5）
...
// 获取可燃气体浓度数据
uint16_t gas_get_data()
{
    // 获取ADC通道4的采样值（0-4095）
    uint16_t adc_value = get_adc_value(ADC_CHANNEL_4);  // 注意：使用通道4
    
    // 将ADC值转换为气体浓度百分比
    uint32_t Percentage_gas = (adc_value / 4095.0) * 100;
    
    return Percentage_gas;
}