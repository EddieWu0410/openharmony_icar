...
osThreadId_t t_newThread(char *name, osThreadFunc_t func, char *arg)
{
    osThreadAttr_t attr = {
        name, 0, NULL, 0, NULL, STACK_SIZE, osPriorityNormal, 0, 0
    };
...
{
    osThreadId_t tid = t_newThread("test_thread", threadTest, "This is a test thread.");
...
static void ThreadTestTask(void)
{
    ...
    printf("\n#####################################################\n");
    printf("//applications/slhsp104_demo: ThreadTestTask\n");

    if (osThreadNew((osThreadFunc_t)rtosv2_thread_main, NULL, &attr) == NULL) {
        printf("[ThreadTestTask] create rtosv2_thread_main: NG\n");
    } else {
        printf("[ThreadTestTask] create rtosv2_thread_main: OK\n");
    }
    printf("#####################################################\n\n");
}
