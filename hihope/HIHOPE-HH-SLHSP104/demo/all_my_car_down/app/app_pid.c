...
void PID_Calc_Motor(motor_data_t *motor)
{
    ...
    for (i = 0; i < MAX_MOTOR; i++)
    {
        ...
        // printf("motor->speed_pwm[i] :%.03f",motor->speed_pwm[i]);
    }
}
...
#define LEN 12
    ...
    {
        data_buffer[0] = 0xFF;
        data_buffer[1] = 0xFC - 1;
        data_buffer[2] = LEN - 2; // 数量
        data_buffer[3] = 0x13;    // 功能位0x13
        data_buffer[4] = index;
        if (index == 5)
        {
            data_buffer[3] = 0x14; // 功能位
